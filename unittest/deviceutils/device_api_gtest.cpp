...
TEST_F(DeviceApiTestFixture, TestName_GetServURL_SuccessStatered_DebugServices_Enabled)
{
    ...
    // Non-PROD builds are always unlocked; isDebugServicesEnabled() is not called for VBN
...
TEST_F(DeviceApiTestFixture, TestName_GetServURL_SuccessStatered_Prod_DebugServices_Enabled)
{
    ...
    // For ePROD unlock: getDeviceTypeRFC must return "test" and LABSIGNED_ENABLED must be "true"
    ...
    EXPECT_CALL(*g_DeviceUtilsMock, getDevicePropertyData(StrEq("LABSIGNED_ENABLED"), _, _))
                .Times(1)
                .WillOnce(Invoke([](const char*, char* data, int size) -> int {
                    if (size > 0) {
...
TEST_F(DeviceApiTestFixture, TestName_GetServURL_SuccessStatered_Prod_DebugServices_Disabled)
{
    ...
    // getDeviceTypeRFC and LABSIGNED_ENABLED are still called for ePROD even when dbgServices=false
    EXPECT_CALL(*g_DeviceUtilsMock, getDeviceTypeRFC(_, _))
                .Times(1)
                .WillOnce(Invoke([](char* deviceType, size_t size) {
                    if (size > 0) {
    ...
    EXPECT_CALL(*g_DeviceUtilsMock, getDevicePropertyData(StrEq("LABSIGNED_ENABLED"), _, _))
                ...
                .WillOnce(Invoke([](const char*, char* data, int size) -> int {
                    ...
                    return 0; // UTILS_SUCCESS - but dbgServices=false prevents unlock
                }));
    ret = system("echo \"https://www.statered.com\" > /tmp/stateredrecovry.conf");
...
// Tests for new isSecureDbgSrvUnlocked() logic: PROD + wrong device type stays locked
TEST_F(DeviceApiTestFixture, TestName_GetServURL_Locked_Prod_WrongDeviceType)
{
    char output[64];
    int ret;
    char servUrl[]="https://www.tr181Rfc.com";
    ...
    ret = system("echo \"BUILD_TYPE=PROD\" > /tmp/device_gtest.prop");
    EXPECT_CALL(*g_DeviceUtilsMock, isDebugServicesEnabled()).Times(1).WillOnce(Return(true));
    // deviceType is "prod" (not "test"), so ePROD remains locked even with labsigned=true
    EXPECT_CALL(*g_DeviceUtilsMock, getDeviceTypeRFC(_, _))
                .Times(1)
                .WillOnce(Invoke([](char* deviceType, size_t size) {
                    if (size > 0) {
    ...
    ret = GetServURL(output, sizeof(output));
    // Should NOT use state-red override (locked); falls through to RFC URL
    EXPECT_EQ(strncmp(output, servUrl, strlen(servUrl)), 0);
    ret = system("rm -f /tmp/stateredrecovry.conf");
    ret = system("rm -f /tmp/device_gtest.prop");
    printf("Server URL = %s\n", output);
...
// Tests for new isSecureDbgSrvUnlocked() logic: PROD + LABSIGNED_ENABLED=false stays locked
TEST_F(DeviceApiTestFixture, TestName_GetServURL_Locked_Prod_LabsignedNotEnabled)
{
    char output[64];
    int ret;
    char servUrl[]="https://www.tr181Rfc.com";
    ...
    ret = system("echo \"BUILD_TYPE=PROD\" > /tmp/device_gtest.prop");
    EXPECT_CALL(*g_DeviceUtilsMock, isDebugServicesEnabled()).Times(1).WillOnce(Return(true));
    // deviceType is "test" but LABSIGNED_ENABLED is "false", so ePROD remains locked
    EXPECT_CALL(*g_DeviceUtilsMock, getDeviceTypeRFC(_, _))
                .Times(1)
                .WillOnce(Invoke([](char* deviceType, size_t size) {
                    if (size > 0) {
...
// Tests for new isSecureDbgSrvUnlocked() logic: PROD + LABSIGNED_ENABLED read failure stays locked
TEST_F(DeviceApiTestFixture, TestName_GetServURL_Locked_Prod_LabsignedReadFail)
{
    char output[64];
    int ret;
    char servUrl[]="https://www.tr181Rfc.com";
    ...
    // getDevicePropertyData fails (returns non-UTILS_SUCCESS), so ePROD remains locked
    EXPECT_CALL(*g_DeviceUtilsMock, getDevicePropertyData(StrEq("LABSIGNED_ENABLED"), _, _))
                .Times(1)

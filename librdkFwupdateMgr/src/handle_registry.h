...
/**
 * @brief Lookup handle by daemon handle ID (alias for registry_find_by_id)
 * 
 * Used by signal handlers to find the handle for a given daemon ID.
 * 
 * @param daemonHandleId Handle ID from daemon
 * @return Handle pointer, or NULL if not found
 */
InternalHandle* registry_lookup_by_daemon_id(uint64_t daemonHandleId);

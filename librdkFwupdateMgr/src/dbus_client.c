 ...
 * 
 * Method signature: CheckForUpdate(t) -> (i)
 * - Input: handlerId (uint64)
 * - Output: result (int32) - 0=success, non-zero=error
 * 
 * Note: This is an async operation. The actual result comes via
 * CheckForUpdateComplete D-Bus signal.
 */
gboolean dbus_call_check_for_update(uint64_t handleId, GError **error)
{
    if (!g_dbus_connection) {
        fprintf(stderr, "[librdkFwupdateMgr] D-Bus not initialized\n");
        if (error) {
    ...
    fprintf(stderr, "[librdkFwupdateMgr] Calling CheckForUpdate: handle ID %" G_GUINT64_FORMAT "\n",
            handleId);
    
    /* Call daemon method: CheckForUpdate(uint64) -> int32 */
    GVariant *result = g_dbus_connection_call_sync(
        g_dbus_connection,
        DBUS_SERVICE_NAME,
        DBUS_OBJECT_PATH,
        ...
        "CheckForUpdate",
        g_variant_new("(t)", handleId),
        G_VARIANT_TYPE("(i)"),  /* Returns int32 */
        G_DBUS_CALL_FLAGS_NONE,
        5000,  /* 5 second timeout */
        NULL,
        error
    ...
    gint32 result_code;
    g_variant_get(result, "(i)", &result_code);
    g_variant_unref(result);
    
    fprintf(stderr, "[librdkFwupdateMgr] CheckForUpdate method call returned: %d\n", result_code);
    
    /* Method call succeeded - actual result will come via signal */
    return TRUE;
}

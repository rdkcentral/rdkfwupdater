...
//	int http_code;
	...
	//Signal handling - currently handling only SIGTERM and SIGINT 
...
while (1) {
	switch (currentState) {
		...
		case STATE_IDLE:
			...
			SWLOG_INFO(" Listening for dbus requests in IDLE state\n");
			sleep(10);
			break;
...
// Cleanup the resources as loop exited
SWLOG_INFO("rdkvfwupgrader Exit\n"); 
log_exit();
exit(ret_curl_code);
